...
class Curve:
    ...
    def ABFromJ(j) -> Tuple:
        ...
        if j.is_zero():
            ...
            return (
                (3 * j) / (1728 - j),
                (2 * j) / (1728 - j),
            )  # formula for A,B with given j (char not 2,3 implicit)

    def __init__(
        self,
        Fq: FqData,
        j,
        aut_grp: Optional[Dict] = None,
        t: Optional[int] = None,
    ...
    def getCoefficients(self):
        ...
        if self.aut_grp is None:
            print(
                f"{Colors.FAIL}Error: Cannot compute twists without automorphism group info{Colors.ENDC}"
            )
            return (self.A, self.B)
...
class GeometricCurve(Curve):
    ...
    def _create(self):
        ...
        if ss != self.is_supersingular:
            print(
                f"{Colors.WARNING}Discrepancy in supersingularity check for j={self.j}: by t={self.is_supersingular}, by is_supersingular()={ss}{Colors.ENDC}"
            )
