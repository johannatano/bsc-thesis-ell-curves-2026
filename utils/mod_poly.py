...
def _classical_modular_polynomial(l, j=None):
    r"""
    ...
    R = parent(j)['Y']
    Y = R.gen()
    print(f"Computing modular polynomial for ell={l} and j={j}...")
    ...
    if l in _cache:
        print(f"Using cached modular polynomial for ell={l}")
        return _cache[l](j, Y)
    try:
        Phi = _db[l]
        print(f"Using modular polynomial from database for ell={l}")
    ...
    try:
        ...
        print(f"Using instantiated modular polynomial from PARI for ell={l} and j={j}")
        return R(pari_Phi)
    except PariError:
        pass
    except TypeError:
    ...
    print(f"Warning: modular polynomial for ell={l} is not in the database and computing it on the fly is not yet implemented. Falling back to computing the generic polynomial and evaluating it, which may be very slow.")
    return classical_modular_polynomial(l)(j, Y)
...
_classical_modular_polynomial.set_cache_bound = _set_cache_bound
